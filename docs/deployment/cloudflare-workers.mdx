...
<Aside type="note">
  On Cloudflare Workers, OIDC endpoints are mounted at the root (`/`).
...
| `STUBIDP_ISSUER`        | _(Optional)_ Override the issuer URL — auto-detected from the request URL if omitted |
...
| `STUBIDP_ISSUER`        | Variable | _(Optional)_ Issuer URL override                                  |
