...
available via the [UserInfo endpoint](/reference/endpoints#userinfo) or the ID token. Some providers instead issue
access tokens as JWTs carrying identity claims directly, which resource servers can validate locally without an
extra UserInfo call. Set `--access-token-format jwt` (or `STUBIDP_ACCESS_TOKEN_FORMAT=jwt`) to match that behavior.
...
<Aside type="tip">
  ID token claims are controlled separately by `--scopes` / `--claims`. Access token claims always include the full
  identity claim set regardless of requested scope, since it mirrors what a real resource server would expect.
</Aside>
